...
var TodoStore = Fluxxor.createStore({
  ...
  onAddTodo: function(payload) {
    ...
    this.undoStates = Immutable.List(this.undoStates.concat({ "list": JSON.stringify(todos), "item": JSON.stringify(payload)}));
  ...
  onLoadTodos: function(todos) {
    ...
    this.undoStates = Immutable.List([JSON.stringify(todos)]);
  ...
  onUndoTodos: function() {
    ...
    this.map = this.map.set('todos', JSON.parse(this.undoStates.get(this.undoStates.size-1)));
